...
// Map languages to stable regional voices
...
export async function POST(req: Request) {
  try {
    ...
    const voiceConfig = VOICE_MAP[languageCode] || VOICE_MAP.en;
    ...
    if (data.error) {
      console.error("Google TTS Error Response:", data.error);
      return NextResponse.json({ error: data.error.message }, { status: 500 });
    }

    if (!data.audioContent) {
      console.warn("Google TTS returned no audio content");
      return NextResponse.json({ error: "Empty audio content from Google" }, { status: 500 });
    }
